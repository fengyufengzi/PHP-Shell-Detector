...
class SignatureNasty:
   """Generator that searches a given file for nasty expressions,use another signature library(499)"""
   ...
   def printer(self, count):
       ...
       return
class ShellSig:
    """Generator that searches a given file for particular expressions,use another signature library(499)"""
    def __init__(self):
        """Instantiate the results array."""
        self.results = []
    def calculate(self, data, filename):
        ...
        lenmatches = 0
        for sig in fp.readlines():
            sig = sig.split('#')
            #print sig[0]
            try:
                #regsig = re.compile(sig[0])
                sig[0] = sig[0].replace('+','')
                matches = re.findall(sig[0],base64data)
                if len(matches) == 0:
                    matches = re.findall(sig[0],data)
                #print len(matches)
            except:
                print sig[0]
                print len(matches)
            ...
            self.results.append({"filename":filename,"value":len(matches),"shellname":sig[1]})
            lenmatches = lenmatches + len(matches)
            #print lenmatches
            #print '---------------------------------------'
        return lenmatches
    ...
    def printer(self, count):
        """Print all of the  signature count match files for a given search"""
        print "\n[[ All of  shellsignature match counts ]]"
        for x in range(count):
            print ' {0:>7}        {1}       {2}'.format(self.results[x]["value"], self.results[x]["filename"],self.results[x]["shellname"])
        return
class CreateTime:
    """find the files been created not during the regular time"""
    def __init__(self):
        """Instantiate the results array."""
        self.results = []
    def calculate(self,path): # grouping by the ctime status:the stats of files; dtime:time period;fnum:minnumber of files
        stats = []
        ...
        #print stats
        fsDict = {} # ctimes of all the files
         #
        for f in stats:
            filepath = f['path']
            stat = f['stat']
        ...
        for i in fsDict.items():
            ...
            if tmp.keys() != []:
                for k in tmp.keys():
                    flag = False
                    #print '-------------------'
                    if (i[1] - k >= timedelta(minutes = -dtime)) and (i[1] - k <= timedelta(minutes = dtime)):
                        #print '------------'
                        tmp[k].append(i[0])
                        flag = True
                        break
        ...
        for i in tmp.keys():
            ...
            if len(tmp[i]) < fnum:
                self.results.append({'filename':i,'value':len(tmp[i]),'filesname':tmp[i]})#filename:time period  ; value:the numbers of created in the same period ; filesname: the filenames
            else:
                self.results.append({'filename':i,'value':len(tmp[i]),'filesname':'ignored'})
    ...
    def sort(self):
        ...
        #self.results.reverse()
    def printer(self, count):
        """Print the top CreateTime count match files for a given search"""
        print "\n[[ Top createtime different ]]"
        if (count > len(self.results)): count = len(self.results)
        for x in range(len(self.results)):
            #print self.results[x]
            if self.results[x]['filesname'] != 'ignored':

                print   ' {0:>7}        {1}         {2}'.format(self.results[x]["value"], self.results[x]["filename"],self.results[x]['filesname'])
...
if __name__ == "__main__":
   ...
   parser.add_option("-T", "--createtime",
       action="store_true",
       dest="is_createtime",
       default=False,
       help="Run create time test",)
   ...
   if options.is_all:
       ...
       tests.append(ShellSig())
       tests.append(Compression())
       tests2.append(CreateTime())
   else:
   ...
   fileIgnoreCount = 0
   for test in tests2:
       calculated_value = test.calculate(args[0])
